...
class Conversation(Widget):
    ...
    def change_channel(self, new_channel_name):
        """Désabonne de l'ancien canal et s'abonne au nouveau."""
        if new_channel_name == self.channel:
            return False
...
class ChatApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with FocusableContainer(id="conversation_box"):
            yield MessageBox(
                "Bienvenue ! Tapez /channel <nom> pour changer de salon.",
                "INFO : ",
            )
    ...
    async def listen(self):
        # La boucle listen de Redis est robuste : elle continue même après un unsubscribe/subscribe
        for m in self.conversation.subscriber.listen():
            if m["type"] == "message":
                conversation_box = self.query_one("#conversation_box")
                # On utilise call_from_thread pour interagir avec l'UI depuis un thread worker
    ...
    def add_message_to_ui(self, text):
        """Helper pour ajouter un message de manière thread-safe."""
        conversation_box = self.query_one("#conversation_box")
    ...
    async def process_conversation(self) -> None:
        ...
        if val.startswith("/username "):
            new_name = val.replace("/username ", "").strip()
            if new_name:
        ...
        # --- NOUVEAU : GESTION DU CHANGEMENT DE CANAL ---
        if val.startswith("/channel "):
            new_channel = val.replace("/channel ", "").strip()
            if new_channel:
